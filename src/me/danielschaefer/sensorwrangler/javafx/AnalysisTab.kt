...
import me.danielschaefer.sensorwrangler.javafx.popups.AddAnalysisPopup
...
class AnalysisTab(parentStage: Stage) : Tab("Analysis") {
    ...
    init {
        content = HBox().apply {
            ...
            chartList = ListView<Chart>().apply {
                items.setAll(App.instance.wrangler.analysis)
                App.instance.wrangler.analysis.addListener(
                    ListChangeListener {
                        it.next()
                ...
                selectionModel.selectedItemProperty().addListener(
                    ChangeListener { _, _, selectedChart ->
                        ...
                        val removeChartButton = Button("Remove Chart").apply {
                            setOnAction {
                                App.instance.wrangler.analysis.remove(selectedChart)
                                if (App.instance.wrangler.analysis.size > 0) {
            ...
            val addChartButton = Button("Add Analysis").apply {
                setOnAction {
                    AddAnalysisPopup(parentStage, this@AnalysisTab)
                }
            }
